...
int8_t move_base(int8_t direction) {	
	...
	if (direction == MOVE_LEFT){
		...
		// Move the base (only to the right at present)
		if (basePosition < 7){
			basePosition++;
		}
...
void new_asteroids(void){
	//Add a new asteroid in the top row (in a random column not already occupied).
	uint8_t m,n;
	do {
		// Generate random x position - somewhere from 0
		// to FIELD_WIDTH - 1
		m = (uint8_t)(random() % FIELD_WIDTH);
		// Generate random y position - somewhere from 3
		// to FIELD_HEIGHT - 1 (i.e., not in the lowest
		// three rows)
		
		n = (uint8_t)(FIELD_HEIGHT-1);
		
		
	} while(asteroid_at(m,n) != -1 && projectile_at(m,n) != -1);
	// If we get here, we've now found an x,y location without
	// an existing asteroid - record the position
	asteroids[numAsteroids] = GAME_POSITION(m,n);
	numAsteroids++;
	redraw_asteroid(asteroid_at(m,n), COLOUR_ASTEROID);
}
...
void hit(void){
	...
	while(projectileNumber < numProjectiles) {
		...
		if(asteroid_at(x,y)!=-1){
			// Projectile is at the asteroid i position
			// Remove the projectile
			remove_projectile(projectileNumber);
			// Remove the asteroids
			remove_asteroid(asteroid_at(x,y));
			//1 is added to the score each time a projectile hits an asteroid.
			add_to_score(1);
			new_asteroids();
		}
		projectileNumber++;
...
void hit_base(void){
	...
	for(i=0; i < numAsteroids; i++) {
		// Get the current position of the asteroids
		
		if (asteroids[i] == GAME_POSITION(basePosition,1) || asteroids[i] == GAME_POSITION(basePosition-1,0) || asteroids[i] == GAME_POSITION(basePosition+1,0)){
			//Removed asteroids when they reach the base station.
			remove_asteroid(i);
